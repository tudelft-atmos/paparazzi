...
static inline void ahrs_int_get_euler_from_accel_mag(struct Int32Eulers* e, struct Int32Vect3* accel, struct Int32Vect3* mag) {
  ...
  const float fg0 = sqrt(accel->x*accel->x + accel->y*accel->y + accel->z*accel->z);
  //  printf("# atan float %f\n", DegOfRad(fphi));

  // Definitions from AeroSpace Euler Definition
  const float ftheta = - asin(accel->x / fg0);
...
static inline void ahrs_int_get_euler_from_accel_mag_backup(struct Int32Eulers* e, struct Int32Vect3* accel, struct Int32Vect3* mag) {
  //  DISPLAY_INT32_VECT3("# accel", (*accel));
  const float fphi = atan2f(-accel->y, -accel->z);
  //  printf("# atan float %f\n", DegOfRad(fphi));
  e->phi = ANGLE_BFP_OF_REAL(fphi);
